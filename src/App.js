...
const Card = ({ children, style = {}, onClick }) => (
  <div onClick={onClick} style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 12, padding: "1rem 1.25rem", cursor: onClick ? "pointer" : "default", ...style }}>{children}</div>
);
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, fontSize: 20, fontWeight: 500, color: "var(--color-text-primary)" }}>
            <div style={{ width: 10, height: 10, borderRadius: "50%", background: G.teal }} />MathCore
          </div>
  ...
  return (
    <div style={{ background: "var(--color-background-primary)", borderBottom: "0.5px solid var(--color-border-tertiary)", padding: "0 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: 52, position: "sticky", top: 0, zIndex: 100 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 15, fontWeight: 500, color: "var(--color-text-primary)" }}>
        <div style={{ width: 8, height: 8, borderRadius: "50%", background: G.teal }} />MathCore
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>{profile?.name || profile?.email}</span>
        <Badge color={profile?.role === "teacher" ? "blue" : "teal"}>{profile?.role === "teacher" ? "教师" : "学生"}</Badge>
  ...
  return (
    <div style={{ padding: "1.5rem", maxWidth: 900, margin: "0 auto" }}>
      <Card style={{ marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 22, fontWeight: 500, marginBottom: 6, color: "var(--color-text-primary)" }}>你好，{profile?.name || "同学"} 👋</div>
...
function KnowledgePage({ setPage }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", gap: 12, padding: "1.5rem", maxWidth: 1100, margin: "0 auto" }}>
      <Card style={{ padding: "10px 0", height: "fit-content" }}>
      ...
      <Card>
        ...
        {["Newton法收敛条件", "Lagrange插值误差估计", "LU分解适用条件"].map((q, i) => (
          <div key={i} style={{ padding: "10px 0", borderBottom: "0.5px solid var(--color-border-tertiary)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 6 }}><Badge color="red">错误</Badge><Btn size="sm">重做</Btn></div>
          </div>
        ))}
      </Card>
    </div>
...
function TeacherPage({ setPage }) {
  ...
  const selectStyle = { width: "100%", fontSize: 13, padding: "8px 10px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, background: "var(--color-background-primary)", color: "var(--color-text-primary)", fontFamily: "var(--font-sans)" };
      ...
      {tab === "AI 出题" && (
        <Card>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 80px auto", gap: 8, marginBottom: 16, alignItems: "end" }}>
            ...
            <Btn variant="primary" onClick={generateQuestions} disabled={aiLoading}>{aiLoading ? "生成中…" : "生成"}</Btn>
          </div>
