...
${vizProfile.extraConstraint ? "\n━━━ 本轮模型专属约束（必须遵守）━━━\n" + vizProfile.extraConstraint + `\n· 本轮允许的 structure：${vizProfile.preferredStructures.join(" / ")}\n· 本轮最多 ${vizProfile.maxVizPerReply} 个 [VIZ:...]\n` : ""}`;
  ...
  // ── 2A：精简版 —— 软指导 AI 用 [[X]] 标记，不强求。
  // ⚠️ 之前写了 ~30 行严格 prompt + "..." 引号示例 + 80 个名字逐个引号，
  //    把 8B 小模型 context 挤爆导致整段回复乱码（用户实测："A" "I" 引号到处都是、
  //    \[..\] 不闭合、文字重复）。现在客户端有 autoMarkTopics 兜底，prompt 只是
  //    锦上添花，简短即可。
  if (isChatMode && Array.isArray(availableTopics) && availableTopics.length > 0) {
    const cleanList = availableTopics
      .filter((n) => typeof n === "string" && n.trim().length > 0)
      .map((n) => n.trim())
      .slice(0, 30); // 上限 30 条避免 prompt 爆量
    const list = cleanList.join("、");
    systemPrompt += `

【知识点列表】${list}
如果回答中提到列表里的名字，用 [[名字]] 双方括号包起来（公式 $...$ 内不要包）。`;
  }
