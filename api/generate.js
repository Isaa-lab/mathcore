...
${vizProfile.extraConstraint ? "\n━━━ 本轮模型专属约束（必须遵守）━━━\n" + vizProfile.extraConstraint + `\n· 本轮允许的 structure：${vizProfile.preferredStructures.join(" / ")}\n· 本轮最多 ${vizProfile.maxVizPerReply} 个 [VIZ:...]\n` : ""}`;
  ...
  // 错题靶向变式块：v2，按结构化诊断（wrongStep / misconception / weakTopics）来设计变式
  // 设计目标：
  //   · 不是"换个数字让学生再做对一次"——而是"在同一步骤上再次设陷阱"，逼学生真的理解
  //   · 把 misconception 直接编码进干扰项（学生如果还信以为真的错命题，就还会选错选项）
  //   · wrongCount ≥ 2 时升级到"多步推理 + 多个相邻陷阱并存"，避免侥幸蒙对
  const errorContextBlock = (() => {
    if (!Array.isArray(errorContext) || errorContext.length === 0) return "";
    const TAG_STRATEGY = {
      formula:    "把"公式记错"作为干扰核心：选项里放公式的常见错误变体（漏系数、错指数、漏负号、记成相邻公式）。",
      definition: "把"定义"作为干扰核心：选项之一是定义的"必要条件但非充分"或"反过来也成立"的伪命题。",
      sign:       "把"正负号 / 符号顺序"作为干扰核心：选项之一漏负号、之一颠倒了符号，让学生再次掉进符号陷阱。",
      boundary:   "把"边界 / 特殊情形"作为干扰核心：题干给一个看似平凡但实则触发特殊情形（如 g(y)=0 / 零向量 / 零矩阵 / 不可逆 / 边界点）的设置。",
      concept:    "把"概念边界"作为干扰核心：制造和正确概念形似的相邻概念（比如"行阶梯"vs"简化行阶梯"），错误选项就是相邻概念的特征。",
      condition:  "把"前提条件"作为干扰核心：题干故意"看起来"满足某定理的形式，实则有一个条件不满足；选项之一假设条件成立后的结论。",
      careless:   "保持原题方法不变，但题目设计成"中间步骤容易写错符号或单位"的形式；解析里逐步标注容易丢分的地方。",
      algebra:    "把"代数变形"作为干扰核心：在通分、因式分解、移项、合并同类项处设陷阱；选项之一是常见的变形错误结果。",
      derivation: "把"中间步骤跳错"作为干扰核心：题干让学生从条件推到结论，干扰项是"跳过某步直接代入"得到的错误结果。",
    ...
    const lines = errorContext.slice(0, 6).map((e, i) => {
      ...
      const escalate = (Number(e.wrongCount) >= 2)
        ? `\n    - **难度升级**：本题已错 ${e.wrongCount} 次——必须把这道变式设计成"多步推理 + 至少 2 处可踩同一坑"，避免学生侥幸蒙对。`
        : "";
      return `· 错题 ${i + 1}：${String(e.question || "").slice(0, 220)}
    - 正确答案：${e.correctAnswer || "未知"}
    - 学生作答：${e.userAnswer || "(未作答)"}
    - 错因 tag：${tags.join(", ")}${e.wrongStep ? `\n    - 在哪一步走错：${String(e.wrongStep).slice(0, 200)}` : ""}${e.misconception ? `\n    - 学生的错信念：${String(e.misconception).slice(0, 200)}` : ""}${e.correctPath ? `\n    - 正确思路要点：${String(e.correctPath).slice(0, 200)}` : ""}${e.remedyFocus ? `\n    - 重点补救子技能：${String(e.remedyFocus).slice(0, 100)}` : ""}${Array.isArray(e.weakTopics) && e.weakTopics.length ? `\n    - 薄弱知识点：${e.weakTopics.slice(0, 3).join(" / ")}` : ""}
    - 变式策略：${strategy}${escalate}`;
    }).join("\n");
    return `==================================================
【优先约束 · 错题靶向变式 OVERRIDE】
==================================================
本次出题是错题专项变式训练。下面是用户最近做错的题 + AI 结构化诊断：
${lines}

铁律：
1. 每道新题必须**对应上述某道错题**，并按其"变式策略 + 在哪一步走错 + 学生的错信念"来设计选项。
2. **干扰项设计**：至少有一个错误选项必须**直接体现学生的错信念**——他如果还信那个错命题，就还会选错。
3. **不要换皮做对**：禁止仅替换数字 / 函数名 / 字母而保留题型。要切换情境（不同函数 / 不同矩阵 / 不同条件 / 不同上下界），但**坑的位置要落在同一步骤**（如：原题在符号顺序上栽，新题也在某个符号顺序的位置上栽）。
4. **解析必须有"对照说明"**：开头一句话写"这道题对应你之前那道错题里 [wrong_step / misconception 简述]"，再写正确推导。
5. wrongCount ≥ 2 的错题**多产 1.5x 道变式**，并按上面的"难度升级"指示做更复杂的题。
6. 输出题数按下方 LAYER 1-5 中要求的数量。
