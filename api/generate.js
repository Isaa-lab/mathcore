...
判断题只需 question / question_en / answer（"正确"/"错误"）/ explanation / explanation_en / type。`;
  ...
  const HANDLER_BUDGET_MS = 58500; // 60s 硬上限，留 1.5s 余量给收尾返回
  // 满页手写 OCR 用 qwen-vl-max（更准但更慢），尽量顶满预算；图已在前端压到 2000px。
  // 文本类（题目提取/批改/解题）是"整份卷子转 JSON"的重活，给到 20s。
  const PER_PROVIDER_MS = hasVisionMessages ? 58000 : 20000;
