...
async function runHandler(req, res) {
  ...
  // ── 防御性输入校验 ──
  // 这些早期 return 是为了避免畸形输入往下传，让深处的字符串模板 / regex / JSON.parse 出乎意料地 crash。
  // 这些 crash 不会被外层 try/catch 兜（事件循环里的 promise rejection / 同步 throw 在 await 之外的等），
  // 直接造成 Vercel FUNCTION_INVOCATION_FAILED。
  const body = req.body || {};
  if (typeof body !== "object" || Array.isArray(body)) {
    return res.status(400).json({ error: "请求体必须是 JSON 对象" });
  }
  // chat 模式下空 question 会让下游 messages 数组没有 user 消息，部分 provider 直接 500
  if ((body.mode === "socratic" || body.mode === "chat" || body.mode === "tutor") && !String(body.question || "").trim()) {
    return res.status(400).json({ error: "chat 模式下 question 字段不能为空" });
  }
  // conversationHistory 必须是数组（如果给了），否则下游 .slice(-12).map(...) 会 crash
  if (body.conversationHistory != null && !Array.isArray(body.conversationHistory)) {
    return res.status(400).json({ error: "conversationHistory 必须是数组" });
  }
  // 超长 conversationHistory 会让 prompt 爆到 100KB+，触发 OOM
  if (Array.isArray(body.conversationHistory) && body.conversationHistory.length > 40) {
    body.conversationHistory = body.conversationHistory.slice(-40);
  }
...
所有数学符号、公式、表达式都 MUST 被 LaTeX 界定符包裹，否则前端会把它当纯文本显示（已多次出事故）：
  ...
  · **绝对禁止：把一个 \\frac / \\sqrt / \\binom 的两个参数拆到不同的 $...$ 里**
    ❌ 错误："$\\frac{f(x_n)}\${$\\frac{f(x_n) - f(x_{n-1})}{x_n - x_{n-1}}$}"
       （分子 + 分母被拆成两段 $..$，KaTeX 报 "missing argument" 渲染碎掉）
    ❌ 错误："$\\frac{1}{$x+1$}"   ❌ 错误："$\\binom{n}\${$k$}"
    ✅ 正确：整个表达式包在同一对 $..$ 里：
       "$x_{n+1} = x_n - \\frac{f(x_n)}{\\frac{f(x_n) - f(x_{n-1})}{x_n - x_{n-1}}}$"
    判断标准：每个 \\frac 后面**两组花括号必须紧贴**，绝不允许中间出现 $。
...
${vizProfile.extraConstraint ? "\n━━━ 本轮模型专属约束（必须遵守）━━━\n" + vizProfile.extraConstraint + `\n· 本轮允许的 structure：${vizProfile.preferredStructures.join(" / ")}\n· 本轮最多 ${vizProfile.maxVizPerReply} 个 [VIZ:...]\n` : ""}`;
  ...
  if (isChatMode && Array.isArray(availableTopics) && availableTopics.length > 0) {
    // 列表太长会爆 prompt；超过 80 条就截断
    const cleanList = availableTopics
      .filter((n) => typeof n === "string" && n.trim().length > 0)
      .map((n) => n.trim())
      .slice(0, 80);
    const list = cleanList.map((n) => `"${n}"`).join("、");
    systemPrompt += `

━━━ 【⚠️ 强制：知识点可点击标注规则】━━━
本资料的知识库里已经有以下 ${cleanList.length} 个知识点：${list}

你的回答里**只要出现上述列表中任何一个名字（或它的子串）**，**必须**用双方括号包裹：
  ✅ "要求特征值，先解 [[特征方程]]，把 [[行列式]] 展开后得到 [[特征值]] 的多项式..."
     ↑ 即使 [[特征方程]] / [[行列式]] / [[特征值]] 都在同一段，每一个首次出现都要包
  ✅ "[[矩阵]] 的对角元素相加叫迹..."
  ❌ "要求特征值，先解特征方程，把行列式展开..." ← 没包裹，全部失效

具体规则：
1. **覆盖率优先**：宁可多包一点也不要漏；列表里出现的名字**至少在首次提到时必须包**
2. 子串也算命中：如果列表里有"矩阵特征值"，你写到"特征值"时也要包成 [[特征值]]（前端会做模糊匹配）
3. 同一段话同一个名字只包一次（首次），后续重复不包
4. 数学公式 $...$ 内部不要包 [[..]]
5. 列表外的概念不要硬包

自检：写完答案后，扫一遍列表里每个名字，确认你的回答中**首次提到**它的位置都用了 [[..]] 包裹。漏一个都算违反规则。
`;
  }
  ...
  const errorContextBlock = (() => {
    ...
    // ⚠️ 这些字符串里嵌套了 ASCII 双引号 "..."，外层必须用 ` 或 '，否则模块加载时 SyntaxError，
    //    整个 /api/generate 端点直接挂掉（用户看到 FUNCTION_INVOCATION_FAILED）。
    const TAG_STRATEGY = {
...
==================================================
【LAYER 6 · 反"简单题"强审查 — 任意 AI 出题都必须过这关】
==================================================
以下类型题目属于"低质量题"，在输出前必须主动识别并强制重写，禁止出现在最终输出中：

❌ 纯定义复述题（例："什么是特征值？A.矩阵的对角元素 B.满足Ax=λx的λ C...."）
   → 这类题考的是背定义，不考理解。必须改写成"在以下情形中，特征值有何特征？"或"用特征值计算xxx"
❌ 是/否判断（例："以下关于XX的描述，哪个是正确的？"后面接教材原句）
   → 这类题没有计算过程，学生靠死记可以答对。必须加入计算或推断要素
❌ 选项只差一两个字的"单词辨认题"（例：A.正定 B.半正定 C.负定 D.半负定 → 只靠记名字）
   → 必须在题干中提供具体矩阵或条件，让学生真正做判断
❌ 选项全是文字描述且无法区分"哪个正确"不靠推算（纯靠文字多数原则）
   → 至少有2个选项需要计算验证才能排除

强制要求：每道单选题必须满足以下至少 2 条：
  ✅ 选项中包含至少 1 个 LaTeX 数学表达式（而非纯文字）
  ✅ 正确答案可由数学推导唯一确定（非"背定义"即知）
  ✅ 至少 2 个干扰项是"计算/推导时常犯的错"（而非随机胡答）
  ✅ 题干给出具体数值、矩阵、函数或条件供推算

审查流程（出每题后强制执行）：
1. 看题干：是在考"背名词/背定义"吗？→ 是，则重写
2. 看选项：纯文字描述、无计算、靠语感能选的？→ 是，则重写
3. 看正确答案：仅凭"教材读过这句话"就能答对？→ 是，则重写
4. 通过以上审查 → 才可加入输出

==================================================
【输出前 7 问自检 — 任意一项不过就整道重写】
==================================================
...
Q7 这道题是否通过了 LAYER 6 反"简单题"审查（答对不需要推算 → 必须重写）？
