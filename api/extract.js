...
- 题型分配：至少 ${Math.max(2, Math.floor(count * 0.7))} 道单选题，至少 1 道判断题`;
  ...
  // ── Per-provider prompt builders ────────────────────────────────────────
  // 不同 LLM 在中文 / JSON 严格度 / 长上下文 / 推理风格上各有强弱；用同一份 prompt 灌给所有
  // provider 会浪费它们各自的优势。这里给每家做一行"开场白 + 一两条针对性 tip"，
  // 共享主体保持完全一致，避免维护爆炸。
  //
  // 设计准则：
  //   gemini    —— 多模态能力强但有时冗长；强调"直接输出 JSON 不解释"
  //   deepseek  —— 中文学术写作强项；让它发挥严谨表达
  //   kimi      —— 长上下文（200K）专家；让它先全局扫描再出题
  //   groq      —— Llama 70B/8B 极速，但 JSON 严格性偏弱；要求纯 JSON / 短解释
  //   anthropic —— Claude，结构化输出最稳；让它"先思考再产出"
  //   custom    —— 用户自配，保守路线
  const PROVIDER_OPENERS = {
    gemini: `你是一位专业数学教授（运行于 Google Gemini）。教材原文可能是英文，但所有输出内容必须使用中文。
【Gemini 专属指令】请直接产出严格 JSON 对象，禁止用 markdown 代码块包裹，禁止任何说明文字。`,
    deepseek: `你是一位严谨的数学教授（运行于 DeepSeek）。教材原文可能是英文，但所有输出内容必须使用中文。
【DeepSeek 专属指令】请发挥你在中文学术语言上的专长：术语精确、标点规范，
拒绝口语化"咱们""一起来"等表达；最终必须是单个有效 JSON 对象，不带任何 markdown 标记。`,
    kimi: `你是 Moonshot Kimi 助理，以数学教授身份为中国大学生准备题库。教材原文可能是英文，但所有输出内容必须使用中文。
【Kimi 专属指令】请发挥你 200K 长上下文能力：先在心里整体扫一遍当前片段的逻辑结构，
再选 8~14 个最值得拆解的知识点；输出必须是单个 JSON 对象，禁止 markdown 代码块。`,
    groq: `You are a math professor running on Groq (Llama-3.3 70B). Output language: Chinese only.
[Groq-specific] BE STRICT: Output one valid JSON object. No explanations. No markdown fences. No prose before/after.
The 中文 instructions below override any English fallback you might consider.`,
    anthropic: `你是 Claude，正在以数学教授身份工作。教材原文可能是英文，但所有输出内容必须使用中文。
【Anthropic 专属指令】请运用你在结构化输出上的优势：先在内部规划完整的知识点拆分与题目编排，
然后一次性输出严格 JSON。禁止 markdown 代码块。`,
...
仅输出如下 JSON，不附加任何其他文字或 markdown：
{
  ...
  // ── OpenAI-compatible helper (DeepSeek / Kimi / Custom / Groq) ────────────
  // 支持 response_format=json_object 的家厂全部启用，避免拿回 markdown 包裹的 JSON
  const callOpenAICompat = async (baseUrl, key, model, { useJsonMode = true } = {}) => {
    try {
      const body = {
        model,
      ...
      if (useJsonMode) {
        // DeepSeek / Kimi / Groq 都支持 OpenAI 的 response_format JSON 模式
      ...
      // 部分模型不支持 json_object，会返回 400；自动 fallback 一次（去掉 response_format）
      if (r.status === 400 && useJsonMode) {
        return await callOpenAICompat(baseUrl, key, model, { useJsonMode: false });
      }
  ...
  // ── Gemini helper ──────────────────────────────────────────────────────────
  // Gemini 2.0+ 支持 responseMimeType: 'application/json' → 真 JSON 模式，避开 markdown 噪音
  ...
  let responseText = "";
  let apiUsed = "";
  // 真正生效的 provider/model —— 用于回写到 questions.generated_by / material_topics.generated_by
  ...
  if (hasUserKey) {
    ...
    if (effectiveProvider === "groq") {
      ...
      if (!responseText) {
        const m2 = "llama-3.1-8b-instant";
        responseText = await callOpenAICompat("https://api.groq.com/openai/v1", k, m2) || "";
        if (responseText) { actualProvider = "groq"; actualModel = m2; }
      }
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (responseText) {
          ...
          actualModel = ANTHROPIC_MODEL;
          apiUsed = "anthropic(server)";
        }
      }
    } catch (e) {
  ...
  console.log(`[extract] api=${apiUsed} provider=${actualProvider} model=${actualModel} text=${cleanText.length}ch topics=${finalTopics.length} questions=${questions.length}`);

  return res.status(200).json({
    topics: finalTopics,
    questions,
    apiUsed,
    // 给前端的归属信息：会被回写到 questions.generated_by / material_topics.generated_by
