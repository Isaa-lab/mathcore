...
1. topics 数量必须 ≥ **20 条**；少于 20 条 → 你必须回到原文继续拆，把"运算 / 条件 / 反例 / 应用"这类侧面单独立成 topic 再输出。
2. 题目必须能区分"会算"和"只会背"——纯定义复述题、复读课本原句的判断题一律不得出现（详见任务二禁止令）。
3. 至少一半题目要求学生在草稿纸上做 1 步以上的运算 / 推导 / 判定，否则视为题目过基础。
4. 教材元数据 / 前言 / 致谢 / 版本信息 / 出版社 一律不得出题——比如"I am pleased to see the text reach its ninth edition"、"this book is published by …"、"ISBN …"、"本书第九版"。这些是产品页的题，不是数学题。
请把这四条当作合格门槛，不要再缩水交差。\n\n`
    : `【掌握度优先 —— 本批输出用于检验"会不会用"，不是考"知不知道"】
请确保 topics ≥ **20 条**且足够细粒度，并保证题目都需要真正的运算 / 推导 / 判定才能选对。教材前言 / 致谢 / 出版信息禁止出题。\n\n`;
...
【任务一】提取 **20~28 个细粒度**核心知识点，并组织成**有层次的目录结构**（不是扁平列表）。**少于 20 个视为失败**——你必须回到原文，把每条粗概念继续拆出"定义 + 操作 + 条件 + 反例 + 应用"四个侧面，直到达到 20 条以上。
...
【目录结构示意 —— 必须照此组织】：
  ...
  注意：每个 section 至少 2 条 depth=1（小标题级别）+ 3 条 depth>=2（叶子）；
  本批次至少 4~6 个 section，加起来达到 20~28 条 topic。如果原文只覆盖单个章节，
  也务必把该章节内的"定义 / 性质 / 算法步骤 / 边界 / 反例 / 应用 / 易错点 / 与其他方法对比"
  分别立成独立 topic。
...
若你倾向于只输出 < 20 个知识点，**强制自检**：本片段里有没有可以再拆的运算 / 条件 / 反例 / 应用 / 边界？
"分离变量法"这一条就能拆出 5~6 个：判定 / 代数操作 / 积分常数归一 / 丢解情形 / 与齐次方程对比 / 与一阶线性方程对比。
继续拆，直到达到 20 条以上才能输出。
...
仅输出如下 JSON，不附加任何其他文字或 markdown（注意：topics 必须有完整层级字段；topics 总数 ≥ 20；questions 必须有 "knowledge_points" 字段——从 topics 中挑 1~3 条 name 填进去）：
{
  "topics": [
    ...
    { "name": "齐次方程的识别与代换", "summary": "右端能写成 g(y/x) 形式的方程称齐次方程；用 v=y/x 代换可化为可分离方程。", "section": "齐次与一阶线性 ODE", "parent": null, "depth": 1, "prerequisites": [] },
    { "name": "积分因子 μ(x) 的构造", "summary": "对 dy/dx + P(x)y = Q(x)，取 μ(x) = e^{∫P(x)dx} 后两端乘 μ；左端化为 (μy)' = μQ。", "section": "齐次与一阶线性 ODE", "parent": null, "depth": 2, "prerequisites": [] },
    { "name": "Bernoulli 方程化线性技巧", "summary": "形如 y' + P y = Q y^n（n≠0,1），令 v = y^{1-n} 化为线性方程；要小心 1-n 的符号。", "section": "齐次与一阶线性 ODE", "parent": "积分因子 μ(x) 的构造", "depth": 3, "prerequisites": ["积分因子 μ(x) 的构造"] }
  ],
  "questions": [
  ...
  const isMetaBookQuestion = (qText, opts) => {
    ...
    // 英文教材前言 / 致谢 / 出版信息常见模式：作者第一人称表态 + 提到 edition / preface / acknowledgments
    // 反例：「I am pleased to see the text reach its ninth edition」是典型前言句
    const englishPreface =
      /\b(?:I am (?:pleased|delighted|honored|grateful)|We (?:are|would like)|It is (?:my|our) (?:pleasure|hope))\b/i.test(blob) ||
      ...
      /\b\w+ edition\b/i.test(blob); // ninth edition / second edition 等
    const hasMath =
      /\$[^$]+\$|\\(?:frac|int|sum|sqrt|prod|lim|partial|begin\{(?:pmatrix|bmatrix|cases)\})/i.test(blob) ||
      /[=≤≥≠→∈∉∀∃∫∑∏√].{0,40}[a-zA-Z0-9]/.test(blob);
    if (englishPreface && !hasMath) return true;
    // 出版社 / ISBN / 版次 / 版权 / 印次
