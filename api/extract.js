...
【任务一】提取 **10~16 个细粒度**核心知识点（**少于 10 个视为失败**，必须把粗概念继续拆开）。每个知识点必须包含：
  - "name"：知识点名称（中文，4~14字，**必须具体到一个可单独出题、可单独讲解的子概念或操作**）
...
× 章节标题 / 元数据：name 不能是"第 X 章"、"线性代数基础"、"绪论"这类目录文字。
...
仅输出如下 JSON，不附加任何其他文字或 markdown（注意：questions 数组里每道题必须有 "knowledge_points" 字段——从 topics 中挑 1~3 条 name 填进去）：
{
  "topics": [
    { "name": "分离变量法的代数操作", "summary": "把 dy/dx = f(x)g(y) 移项为 dy/g(y) = f(x)dx 的具体写法及哪些项不可丢；适用于一阶可分离 ODE。" },
    { "name": "积分常数 C 的归一", "summary": "两端积分后得 H(y) = F(x) + C₀，常用 ln|·| → e^C₀ = C 合并的技巧；C 必须在解集化简后再带边界条件。" }
  ],
  "questions": [
    ...
    { "type": "单选题", "knowledge_points": ["分离变量法的适用条件"], "question": "下列方程中**不能**直接用分离变量法求解的是？", "options": ["A. $\\frac{dy}{dx}=x^2 \\sin y$", "B. $\\frac{dy}{dx}=\\frac{x+y}{x-y}$", "C. $\\frac{dy}{dx}=e^x e^{-y}$", "D. $\\frac{dy}{dx}=y^2/(1+x^2)$"], "answer": "B", "explanation": "分离变量要求右端能写成 $f(x)g(y)$ 形式；B 中 $(x+y)/(x-y)$ 无法因式拆开，需用齐次代换。A、C、D 均可分离。" }
  ]
}`;
