...
【任务一】提取**至少 20 个**（理想 20~25 个）**细粒度**核心知识点 —— 这是硬性要求，少于 20 个视为不合格必须重抽。每个知识点必须包含：
  - "name"：知识点名称（中文，4~12字，必须**具体**到一个可独立讲解 / 可独立出题的概念或方法）
  - "summary"：一句话说明该知识点的核心内容（中文，30~80字，要写出"是什么 + 为什么 / 何时用"）
  - "category"：知识点分类（从以下 5 类中选一个最合适的，不得自造类别）：
      · "核心概念" — 基本定义、术语、符号（如"特征值"、"极限"、"连续性"）
      · "定理性质" — 定理、引理、推论、性质的陈述（如"中值定理"、"正定矩阵性质"）
      · "计算方法" — 具体算法或步骤（如"高斯消元"、"积分因子法"、"Jacobi 迭代"）
      · "公式推导" — 关键公式、恒等式、推导过程（如"行列式展开式"、"Taylor 展开"）
      · "应用技巧" — 解题技巧、判定准则、典型应用（如"特殊矩阵识别"、"收敛性判断"）

【知识点细化要求 —— 出现以下情形视为不合格】：
× **不足 20 条：必须重新拆得更细，把"大类知识点"拆成"定义 / 性质 / 算法 / 推导 / 应用"多个小条目，直到至少 20 条。**
× 过粗：仅写"线性代数"、"矩阵"这种模糊大类。✓ 正确：写"矩阵的初等行变换"、"行最简形矩阵"、"矩阵秩的判定"等具体概念。
× 过短：summary 仅 10 字以内。✓ 正确：必须解释概念的关键性质或使用场景。
× 重复：两条 name 高度相似（如"矩阵运算"和"矩阵的运算"）—— 必须挑更精确的那个。
× 章节标题：name 不能是"第 X 章"、"线性代数基础"这类教材目录文字。

【拆分提示 —— 如何凑够 20 条】：
  · 每个核心概念都可以拆成"定义+核心性质+判定方法+典型反例+应用场景"5 个独立条目
  · 每个重要定理都可以拆成"陈述+条件分析+证明思路+常见用法+反例"
  · 每种计算方法都可以拆成"步骤+收敛性/复杂度+适用场景+常见错误+变体"
  · 教材里出现的每个不同符号 / 术语 / 算子都值得作为独立"核心概念"条目
